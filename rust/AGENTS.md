...
existing style (`parse_args` → `Config`, the `Matcher`, the testable `filter_available` core;
return `ParseError`, never panic). Add tests, keep `cargo test` green.
...
~a minute. Driving the loop from this agent over MCP needs a one-time login (separate: reviews
